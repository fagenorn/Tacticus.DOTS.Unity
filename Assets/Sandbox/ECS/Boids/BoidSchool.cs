    ...
    public partial class BoidSchoolSpawnSystem : SystemBase
    {
            ...
            Entities.WithStructuralChanges().ForEach((Entity entity, int entityInQueryIndex, in BoidSchool boidSchool, in LocalToWorld boidSchoolLocalToWorld) =>
                                                     {
                                                         ...
                                                         var setBoidLocalToWorldJob = new SetBoidLocalToWorld { LocalToWorldFromEntity = localToWorldFromEntity, Entities = boidEntities, Center = boidSchoolLocalToWorld.Position, Radius = boidSchool.InitialRadius };
        ...
        private struct SetBoidLocalToWorld : IJobParallelFor
        {
            ...
            public void Execute(int i)
            {
                ...
                var dir          = math.normalizesafe(random.NextFloat3() - new float3(0.5f, 0f, 0.5f)) * new float3(1f, 0f, 1f);
                var pos          = Center + dir * Radius;
                var localToWorld = new LocalToWorld { Value = float4x4.TRS(pos, quaternion.LookRotationSafe(dir, math.up()), new float3(1.0f, 1.0f, 1.0f)) };
